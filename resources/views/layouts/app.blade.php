...
<body x-data="{ darkMode: localStorage.getItem('darkMode') === 'true', scrolled: false }" 
      :class="{ 'dark': darkMode }" 
    ...
    <header :class="{ 'glass scrolled': scrolled, 'glass': !scrolled }">
        <div class="container nav-content">
            ...
            <nav class="nav-links">
                ...
                <div style="display: flex; align-items: center; gap: 1.5rem; margin-left: 1rem; border-left: 1px solid hsla(var(--border) / 0.5); padding-left: 1.5rem;">
                    <button @click="darkMode = !darkMode; localStorage.setItem('darkMode', darkMode)" 
                            class="dark-mode-toggle" 
                            style="background: none; border: none; cursor: pointer; color: hsl(var(--primary)); display: flex; align-items: center;">
                        <svg x-show="!darkMode" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" style="width: 20px; height: 20px;">
                    ...
                    @auth
                        <a href="/admin" class="btn btn-primary" style="padding: 0.5rem 1.25rem; font-size: 0.75rem;">Dashboard</a>
                    @else
                        <a href="/admin/login" class="btn btn-outline" style="padding: 0.5rem 1.25rem; font-size: 0.75rem;">Login</a>
                    @endauth
                </div>
            </nav>
        </div>
    ...
    <footer>
        <div class="container" style="display: grid; grid-template-columns: 2fr 1fr 1fr; gap: 4rem; text-align: left;">
            <div>
                <a href="/" class="logo" style="margin-bottom: 1.5rem; display: block;">{{ $settings->logo_text ?? 'PAMOJA' }}</a>
            ...
            <div>
                <h4 style="margin-bottom: 1.5rem; text-transform: uppercase; font-size: 0.75rem; letter-spacing: 0.1em; color: hsl(var(--primary));">Quick Links</h4>
            ...
            <div>
                <h4 style="margin-bottom: 1.5rem; text-transform: uppercase; font-size: 0.75rem; letter-spacing: 0.1em; color: hsl(var(--primary));">Contact</h4>
        ...
        <div class="container" style="margin-top: 5rem; pt: 2rem; border-top: 1px solid hsla(var(--border) / 0.5); text-align: center;">
